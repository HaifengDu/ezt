           ...
           <div class="good-content-list">
             <div class="good-item" v-for="(item) in goodList" :key='item.id'>
                <div class="good-item-title">
                  ...
                  <!-- 默认不可以进行编辑  BOH不限制-->
                  <span class="good-item-sort" v-if="InterfaceSysTypeBOH || !materialLimit.showPrice && !materialLimit.editPrice && materialLimit.billsPageType != 'stocktaking'">{{item.price}}元/{{item.utilname}}（{{item.unit}}）</span>
